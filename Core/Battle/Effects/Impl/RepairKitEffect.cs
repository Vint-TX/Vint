...
using Vint.Core.Battle.Effects.Components.Config.Healing;
...
public sealed class RepairKitEffect : DurationEffect, ISupplyEffect, IExtendableEffect {
    ...
    public RepairKitEffect(BattleTank tank, int level = -1) : base(tank, level, MarketConfigPath) {
        ...
        InstantHp = IsSupply
            ? 0
            : Leveling.GetStat<ModuleHealingEffectInstantHPPropertyComponent>(MarketConfigPath, Level);
    ...
    public override async Task Activate() {
        ...
        Entity = new HealingEffectTemplate().Create(Tank.Tanker, Duration);

...
public class DamageCalculator : IDamageCalculator {
    ...
    public async Task<CalculatedDamage> Calculate(
        BattleTank source,
        BattleTank target,
        ...
        bool isTurretHit = weaponHandler is ShaftWeaponHandler { Aiming: true } && IsTurretHit(hitPoint, target.Entities.Tank);
        bool isBackHit = !isTurretHit && (weaponHandler is not IsisWeaponHandler || isEnemy) && IsBackHit(hitPoint, target.Entities.Tank);
        ...
        float effects = GetEffectsMultiplier(source, target, weaponHandler, isSplash, isBackHit, isTurretHit, ignoreSourceEffects);

...
public class TypeCodecInfo(
    ...
    public static bool operator ==(TypeCodecInfo? x, TypeCodecInfo? y) {
        ...
        return x.Type == y.Type && x.Nullable == y.Nullable && x.Varied == y.Varied && x.Attributes.SetEquals(y.Attributes);
    }
        ...
        if (Type.IsGenericType) {
            ...
            if (genericTypeArguments.Length > 1) {
                typeName = genericTypeArguments
                    .Skip(1)
                    .Aggregate(typeName, (current, genericArg) => $"{current}, {genericArg.Name}");
            }
